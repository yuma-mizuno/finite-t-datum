...
def hereditary(candidate):
    ...
    for size in range(min(n-1,4),0,-1):
            ...
            while remaining:
                ...
                if restricted not in VALID[len(T)]:
                    return {'lemma':'sign-pure external columns preserve the principal T-datum',
                            ...
                            'dependency':'Complete polynomial-lift classification under simultaneous positivity in ranks at most four.'}
    return None
...
def leaf_extension(candidate):
    ...
    for sign in range(2):
        ...
        for leaf in range(n):
            ...
            if any(p[i][j] for i in top for j in range(n)) or p[leaf][leaf]:continue
...
def obstruction(candidate):
    return hereditary(candidate) or leaf_extension(candidate)
...
def add_monomial_relations(solver,variables,indices,candidate):
    ...
    for s in range(2):
        zero_rows=[i for i in range(n) if not any(counts[s][i])]
        for i,j in it.combinations(zero_rows,2):
