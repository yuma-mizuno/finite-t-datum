...
string canonical(const Matrix& b,const vector<int>& word,const vector<int>& p){
    ...
    string s;s.reserve(2*r+n*(n-1));for(int length:lengths)put(s,length);
...
void decode(const string& s,Matrix& b,vector<int>& word,vector<int>& p,vector<int>& lengths){
    ...
    for(int i=0;i<n;i++)for(int j=i+1;j<n;j++){b[i][j]=get(s,at)-32768;b[j][i]=-b[i][j];}
}
...
int main(int argc,char** argv){
    ...
    for(int i=0;i<n;i++)for(int j=0;j<n;j++)if(b[i][j]!=-b[j][i])throw runtime_error("Not skew symmetric");
    ...
    bool first=true;for(int x:lengths){if(!first)out<<',';first=false;out<<x;}for(const auto& row:b)for(int x:row)out<<','<<x;
