...
def main():
    ...
    spent=sum(json.loads(x)['wall_seconds'] for x in ledger.read_text().splitlines());remaining=max(0,3600-spent)
    ...
    try:
        ...
        report={'rank':a.rank,'stage':a.stage,'wall_seconds':time.monotonic()-start,'previous_wall_seconds':spent,
                'budget_seconds':3600,'terminated_at_budget':timed_out,'exit_code':child.returncode,'command':a.command}
