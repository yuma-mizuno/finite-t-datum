"""Exact cyclic mutation-loop signatures for the rank-four representatives.

Canonical labels follow the terminal permutation orbits, rooted at the four
...
import sys
sys.path.append(str(__import__("pathlib").Path(__file__).resolve().parent.parent / "rank3"))
...
def main():
    families = [json.loads(line) for line in (HERE / "families.jsonl").read_text().splitlines()]
    witnesses = [{"id":item["id"],"status":"sat","certificate":space["certificate"]} for item in families for space in item["spaces"]]
    results, minima = [], {}
    ...
    print("All computed cyclic mutation-loop equivalence classes are distinct.")
