...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('.record-link').count(),108);
    ...
    assert(links.includes('../../'+records.find(r=>r.id==='r6-c88').provenance.query_path));
    ...
    console.log('PASS: 108 rank-six entries, three-digit navigation, proof, source links, 570-step Dynkin and infinite/surface replays, spectra and mobile layout.');
